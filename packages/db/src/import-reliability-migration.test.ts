...
const migration = readFileSync(
  resolve(process.cwd(), "../../supabase/migrations/0016_import_reliability.sql"),
  "utf8",
);
...
describe("import reliability migration contract", () => {
  it("has independent request replay and source-item uniqueness barriers", () => {
    ...
    expect(migration).toContain("on conflict (household_id, request_key) do nothing");
  ...
  it("keeps claim writes behind RLS and an authenticated-only RPC", () => {
    expect(migration).toContain("alter table import_item_claims enable row level security");
    expect(migration).toContain("for select using (is_household_member(household_id))");
    expect(migration).toContain("revoke all on function confirm_import_v2(jsonb, jsonb) from anon");
    expect(migration).toContain("grant execute on function confirm_import_v2(jsonb, jsonb) to authenticated");
  });

  it("persists explicit direction-scoped learning in the confirmation transaction", () => {
    expect(migration).toContain("create table if not exists source_category_mappings");
    expect(migration).toContain("unique (household_id, source, normalized_label, row_kind)");
    expect(migration).toContain("add column if not exists row_kind transaction_kind");
